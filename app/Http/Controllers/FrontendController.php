...
class FrontendController extends Controller
{
    ...
    public function home()
    {
        ...
        // Bypass installation check - application is already configured
        // if (env('DB_DATABASE') == null) {
        ...
        if ($request->type == 'user') {
            if (Auth::guard('appuser')->attempt($userdata, $remember)) {
                ...
                if (!$setting->user_verify) {
                    ...
                    // Check if there's an intended URL (like checkout page), otherwise redirect to dashboard
                    $intendedUrl = session()->pull('url.intended', '/user-tickets');
            ...
            if (Auth::attempt($userdata, $remember)) {
                if (Auth::user()->hasRole('Organizer')) {
                    if (Auth::user()->status == 1) {
                        ...
                        Auth::logout();
                        return Redirect::back()->with('error_msg', 'Your account is not active. Please contact administrator.');
                    }
                } else {
                    Auth::logout();
    ...
    {
        // Logout from appuser guard if authenticated
        if (Auth::guard('appuser')->check()) {
            Auth::guard('appuser')->logout();
        }
        
        // Also logout from default guard if authenticated
        if (Auth::check()) {
            Auth::logout();
        }
    ...
    public function userRegister(Request $request)
    {
        ...
        $data = $request->all();
        // Map organization_name from form to name field in database
        ...
        // Set name field for both user types
        if ($data['type'] == 'org') {
            // For organizers: use organization name if available, otherwise first_name
            ...
            // For regular users: combine first_name and last_name for name field
        ...
        // Check if there's an intended URL (from booking flow) and preserve it for after login
    ...
    public function LoginByMail($id)
    {
        ...
        if (Auth::guard('appuser')->loginUsingId($id)) {
            ...
            $verify->update();
            $this->setLanguage($user);
            // Check if there's an intended URL (from booking flow), otherwise redirect to dashboard
            $intendedUrl = session()->pull('url.intended', '/user-tickets');
            return redirect($intendedUrl);
        }
    }
    ...
    public function eventDetail($id, $name = null)
    {
        ...
        SEOMeta::setTitle($data->name)
            ...
            ->addMeta('event:category', $data->category ? $data->category->name : 'Uncategorized', 'property')
            ->addKeyword([
                $setting->app_name,
                $data->name,
        ...
        OpenGraph::setTitle($data->name)
            ...
            ->setArticle([
                ...
                'organization' => $data->organization ? $data->organization->name ?? $data->organization->first_name : 'Unknown',
                'catrgory' => $data->category ? $data->category->name : 'Uncategorized',
                'type' => $data->type,
    ...
    public function userTickets()
    {
        ...
        SEOMeta::setTitle('User Dashboard')
            ->setDescription('User dashboard page')
            ->addKeyword([
                $setting->app_name,
                $user->name,
                $user->name . ' ' . $user->last_name,
        ...
        OpenGraph::setTitle('User Dashboard')
            ->setDescription('User dashboard page')
            ->setUrl(url()->current())
            ->addImage(asset($user->imagePath . $user->image));

        JsonLd::setTitle('User Dashboard')
            ->setDescription('User dashboard page')
            ->addImage(asset($user->imagePath . $user->image));

        SEOTools::setTitle('User Dashboard');
        SEOTools::setDescription('User dashboard page');
        SEOTools::opengraph()->setUrl(url()->current());
        SEOTools::setCanonical(url()->current());
        
        (new AppHelper)->eventStatusChange();

        // Get upcoming and past tickets
        $upcomingQuery = Order::with([
                'event' => function($query) {
                    $query->select('id', 'name', 'image', 'start_time', 'type', 'end_time', 'address');
                },
                'ticket' => function($query) {
                    $query->select('id', 'ticket_number', 'start_time', 'name', 'price', 'type');
                },
        ...
        $pastQuery = Order::with([
                'event' => function($query) {
                    $query->select('id', 'name', 'image', 'start_time', 'type', 'end_time', 'address');
                },
                'ticket' => function($query) {
                    $query->select('id', 'ticket_number', 'name', 'type', 'price');
                },
                'organization' => function($query) {
                    $query->select('id', 'first_name', 'last_name', 'image');
                }
        ...
        $ticket['upcoming'] = $upcomingQuery->paginate(10, ['*'], 'upcoming_page');
        $ticket['past'] = $pastQuery->paginate(10, ['*'], 'past_page');

        // Get event dates for calendar
        $eventDates = [];
        foreach ($ticket['upcoming'] as $order) {
            if ($order->event) {
                $eventDates[] = $order->event->start_time->format('Y-m-d');
            }
        }

        // Get liked events and blogs
        $likedEvents = Event::whereIn('id', array_filter(explode(',', $user->favorite)))
        ...
        $likedBlogs = Blog::whereIn('id', array_filter(explode(',', $user->favorite_blog)))
            ->where('status', 1)
            ->orderBy('id', 'DESC')
            ->get();

        $userFollowing = User::whereIn('id', array_filter(explode(',', $user->following)))
            ->where('status', 1)
            ->orderBy('id', 'DESC')
            ->get();

        $wallet = PaymentSetting::first()->wallet ?? 0;
        ...
        $eventDates = $ticket['upcoming']->map(function ($order) {
            return \Carbon\Carbon::parse($order->event->start_time)->format('Y-m-d');
        })->unique()->values()->all();

        return view('frontend.userTickets', compact('likedEvents', 'ticket', 'likedBlogs', 'userFollowing', 'wallet', 'eventDates'));
    }
    ...
    public function userOrderTicketDashboard($id)
    {
        $order = Order::with(['event', 'ticket', 'organization'])->find($id);
        $taxes_id = OrderTax::where('order_id', $order->id)->get();
        // $coupon = Coupon::find($order->coupon_id);
        ...
        $orderchild = OrderChild::where('order_id', $order->id)->get();
        $review = Review::where('order_id', $order->id)->first();
        return view('frontend.userOrderTicketDashboard', compact('order', 'taxes', 'review', 'orderchild'));
    }
    ...
    // User Dashboard Menu Methods
    public function userOrders()
    {
        $orders = Order::with(['event', 'ticket'])->where('customer_id', Auth::guard('appuser')->user()->id)->orderBy('created_at', 'desc')->paginate(10);
        return view('frontend.userOrders', compact('orders'));
    }

    public function userOrderConfirmation($id)
    {
        $order = Order::with(['event', 'customer', 'ticket'])->findOrFail($id);
        
        // Ensure the order belongs to the authenticated user
        if ($order->customer_id != Auth::guard('appuser')->user()->id) {
            abort(403, 'Unauthorized access to order.');
        }
        
        // Get individual ticket details for QR codes
        $orderchild = OrderChild::where('order_id', $order->id)->get();
        
        return view('frontend.userOrderConfirmation', compact('order', 'orderchild'));
    }
    ...
    public function userProfile()
    {
        ...
        SEOTools::setDescription('User profile page');

        return view('frontend.userProfile', compact('user', 'setting'));
    }
    ...
    public function userProfileUpdate(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            if ($user->image && file_exists(public_path('images/upload/' . $user->image))) {
                unlink(public_path('images/upload/' . $user->image));
            }
    ...
    public function userChangePasswordView()
    {
        ...
        SEOTools::setDescription('Change password page');

        return view('frontend.userChangePassword', compact('user', 'setting'));
    }
    ...
    public function userPasswordUpdate(Request $request)
    {
        ...
        return redirect()->route('userChangePassword')->with('success', 'Password changed successfully!');
    }

    public function downloadTicket($id)
    {
        $user = Auth::guard('appuser')->user();
        $order = Order::with(['event', 'ticket', 'organization'])
            ->where('id', $id)
            ->where('customer_id', $user->id)
            ->first();
        ...
        // Check if specific ticket is requested
        $specificTicket = request()->get('ticket');
        if ($specificTicket) {
            // Filter to only the specific ticket
            $orderchild = $orderchild->where('ticket_number', $specificTicket);
            if ($orderchild->isEmpty()) {
                return redirect()->back()->with('error', 'Specific ticket not found');
            }
            $filename = 'ticket-' . $specificTicket . '.pdf';
        } else {
            // Download all tickets
            $filename = 'ticket-' . $order->order_id . '.pdf';
        }
        
        $pdf = FacadePdf::loadView('frontend.ticketPDF', compact('order', 'setting', 'orderchild'))
    ...
    {
        $order = \App\Models\Order::with(['event', 'customer', 'ticket'])->findOrFail($id);
    ...
    public function howItWorks()
    {
        ...
        SEOMeta::setTitle($setting->app_name . ' - How it Works' ?? env('APP_NAME'))
            ->setDescription('Learn how our event ticketing platform works')
            ->setCanonical(url()->current());
            
        SEOTools::jsonLd()->addImage($setting->imagePath . $setting->logo);
        
        return view('frontend.how-it-works', compact('setting', 'activePage'));
    }
    ...
    public function pricing()
    {
        ...
        SEOMeta::setTitle($setting->app_name . ' - Pricing' ?? env('APP_NAME'))
            ->setDescription('View our transparent pricing plans for event organizers')
            ->setCanonical(url()->current());
            
        SEOTools::jsonLd()->addImage($setting->imagePath . $setting->logo);
        
        return view('frontend.pricing', compact('setting', 'activePage'));
    }
    ...
    public function aboutUs()
    {
        ...
        SEOMeta::setTitle($setting->app_name . ' - About Us' ?? env('APP_NAME'))
            ->setDescription('Learn more about our mission and team')
            ->setCanonical(url()->current());
            
        SEOTools::jsonLd()->addImage($setting->imagePath . $setting->logo);
        
        return view('frontend.about-us', compact('setting', 'activePage'));
    }
    public function myTickets()
    {
        ...
        SEOMeta::setTitle('My Tickets')
            ->setDescription('My tickets page')
            ->addKeyword([
                $setting->app_name,
                $user->name,
                $user->name . ' ' . $user->last_name,
        ...
        OpenGraph::setTitle('My Tickets')
            ->setDescription('My tickets page')
            ->setUrl(url()->current())
            ->addImage(asset($user->imagePath . $user->image));

        JsonLd::setTitle('My Tickets')
            ->setDescription('My tickets page')
            ->addImage(asset($user->imagePath . $user->image));

        SEOTools::setTitle('My Tickets');
        SEOTools::setDescription('My tickets page');
        SEOTools::opengraph()->setUrl(url()->current());
        SEOTools::setCanonical(url()->current());
        
        (new AppHelper)->eventStatusChange();

        return view('frontend.myTickets');
    }
}
