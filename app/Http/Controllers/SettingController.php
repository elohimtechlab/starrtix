...
class SettingController extends Controller
{
    ...
    public function toggleFrontend(Request $request)
    {
        $setting = Setting::first();
        
        // Get the frontend_maintenance value from the request
        $frontendMaintenance = $request->input('frontend_maintenance', 0);
        
        // Set the frontend maintenance mode based on the received value
        $setting->frontend_maintenance = (int) $frontendMaintenance;
        $setting->save();
        
        $status = $setting->frontend_maintenance ? 'disabled' : 'enabled';
        return redirect()->back()->withStatus(__("Frontend has been {$status} successfully."));
    }
}
