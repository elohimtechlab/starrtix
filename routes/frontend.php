...
Route::group(['middleware' => ['mode', 'XSS', 'frontend.maintenance']], function () {
    ...
    Route::post('/events/{id}/send-invite', [EventController::class, 'sendInvite'])->name('events.send-invite');
    Route::get('/invite/{token}', [EventController::class, 'showInvite'])->name('invite.show');
    Route::post('/invite/{token}/respond', [EventController::class, 'respondToInvite'])->name('invite.respond');
    ...
    Route::group(['prefix' => 'user'], function () {
        ...
        // Custom admin logout route removed to avoid conflict with Laravel's built-in logout route
    ...
    Route::group(['middleware' => 'checkStatus'], function () {
        ...
        Route::group(['middleware' => 'appuser'], function () {
            ...
            Route::get('/booking-success/{id}', [FrontendController::class, 'bookingSuccess'])->name('booking.success');
            Route::get('/test-success', function() { return 'Test Success Page Working!'; });
            Route::get('/my-orders', [FrontendController::class, 'userOrders'])->name('userOrders');
            Route::get('/my-ticket/{id}', [FrontendController::class, 'userOrderTicket'])->name('userOrderTicket');
            Route::get('/ticket-details/{id}', [FrontendController::class, 'userOrderTicketDashboard'])->name('ticket-details');
            
            // User Dashboard Menu Routes
            Route::get('/user-tickets', [FrontendController::class, 'userTickets'])->name('userDashboard');
            Route::get('/my-tickets', [FrontendController::class, 'myTickets'])->name('myTickets');

            Route::get('/user-order-confirmation/{id}', [FrontendController::class, 'userOrderConfirmation'])->name('userOrderConfirmation');

            Route::get('/user-profile', [FrontendController::class, 'userProfile'])->name('userProfile');
            Route::post('/user-profile-update', [FrontendController::class, 'userProfileUpdate'])->name('userProfileUpdate');
            Route::get('/user-change-password', [FrontendController::class, 'userChangePasswordView'])->name('userChangePassword');
            Route::post('/user-password-update', [FrontendController::class, 'userPasswordUpdate'])->name('userPasswordUpdate');
            Route::get('/download-ticket/{id}', [FrontendController::class, 'downloadTicket'])->name('downloadTicket');
            ...
            Route::group(['prefix' => 'user'], function () {
                ...
                Route::post('/deposite', [WalletController::class, 'deposite'])->name('deposite');
                                Route::any('/wallet/stripe/create-session', [WalletController::class, 'checkoutSession'])->name('wallet.stripe.checkoutSession');
