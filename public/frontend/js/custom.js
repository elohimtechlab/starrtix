...
$(document).ready(function () {
    ...
    // Initialize Place Order button for all payment methods
    function initializePlaceOrderButton() {
        $("#form_submit").off("click").on("click", function(e) {
            e.preventDefault();
            ...
            var requestData = {
                payment: $("#payment").val(),
                payment_token: $("#payment_token").val() || null,
    ...
    // Initialize the Place Order button on page load
    ...
    $(".payments input[type=radio][name=payment_type] ").on(
        ...
        function () {
                                        ...
                                        success: function (data) {
                                            if (data.success == true) {
                                                window.location.href =
                                                    "/user-order-confirmation/" + data.order_id;
                                            } else {
                                                $("#stripe_message").text(
                                                    data.message
                                                );
            ...
            if (this.value == "LOCAL") {
                ...
                $("#paypal-button-container").hide();
                $("#stripeform").hide();
                // Place Order button is now handled by initializePlaceOrderButton()
            }
            if (this.value == "FREE") {
                $("#form_submit").show();
                $("#paypal-button-container").hide();
    ...
    // Create Order - Updated for correct URL routing
    function createOrder(requestData) {
        $.ajax({
            ...
            success: function (data) {
                if (data.success == true) {
                    window.location.href = "/user-order-confirmation/" + data.order_id;
                } else {
...
$(document).ready(function () {
    // PayPal
    $("#paypalWallet").off("click").on("click", function () {
        paypal_sdk
            .Buttons({
                createOrder: function (data, actions) {
                    return actions.order.create({
    ...
    // Razorpay
    $("#razorpayWallet").off("click").on("click", function () {
        var razorpayOptions = {
            key: $("#razor_key").val(),
    ...
    // Stripe
    $("#stripeWallet").off("click").on("click", function () {
        var stripe = Stripe($("#stripePublicKey").val());
    ...
    // Flutterwave
    $("#flutterwaveWallet").off("click").on("click", function () {
        FlutterwaveCheckout({
    ...
    // Initialize wallet Proceed to Pay button
