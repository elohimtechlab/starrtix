...
@section('content')
                                ...
                                <div class="flex msm:flex-wrap xxsm:flex-wrap">
                                    ...
                                    <div class="ml-3">
                                        ...
                                        <p class="font-poppins font-normal text-base leading-6 text-gray">
                                            {{ $data->organization->name ?? $data->organization->first_name }}</p>
                                    </div>
                                </div>
                            </a>
                        </div>
                            ...
                            <div class="border-t pt-4">
                                <div class="flex flex-wrap justify-between items-center gap-4">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <div class="qr-code-container bg-gray-50 p-3 rounded-lg">
                                            <div id="eventQRCode" class="qr-code"></div>
                                        </div>
                                        <div>
                                            <h6 class="font-poppins font-semibold text-lg text-black mb-2">{{ __('Share Event') }}</h6>
                                            <p class="font-poppins text-sm text-gray-600">{{ __('Scan QR code or use social buttons') }}</p>
                                        </div>
                                    </div>
                                    
                                    <!-- Sharing Buttons -->
                                    <div class="flex items-center space-x-3">
                                        <button onclick="shareOnFacebook()" class="share-btn facebook-btn" title="Share on Facebook">
                                            <i class="fab fa-facebook-f"></i>
                                        </button>
                                        <button onclick="shareOnTwitter()" class="share-btn twitter-btn" title="Share on Twitter">
                                            <i class="fab fa-twitter"></i>
                                        </button>
                                        <button onclick="shareOnWhatsApp()" class="share-btn whatsapp-btn" title="Share on WhatsApp">
                                            <i class="fab fa-whatsapp"></i>
                                        </button>
                                        <button onclick="copyEventLink()" class="share-btn copy-btn" title="Copy Link">
                                            <i class="fas fa-link"></i>
                                        </button>
                                        <button onclick="downloadQRCode()" class="share-btn download-btn" title="Download QR Code">
    ...
    <!-- QR Code and Sharing JavaScript -->
    <script>
        ...
        const eventData = {
            ...
            description: {!! json_encode(strip_tags($data->description)) !!}
        };
        ...
        function generateEventQRCode() {
            ...
            try {
                ...
                if (typeof QRCode !== 'undefined') {
                    QRCode.toCanvas(qrContainer, eventData.url, {
                        width: 80,
                        height: 80,
                        margin: 1,
                        color: {
                        ...
                        if (error) {
                            console.log('QRCode library failed, trying fallback');
        ...
        // Fallback QR code generation using external API
        function generateQRCodeFallback() {
            const qrContainer = document.getElementById('eventQRCode');
            if (!qrContainer) return;
            
            const qrImg = document.createElement('img');
            qrImg.src = `https://api.qrserver.com/v1/create-qr-code/?size=80x80&data=${encodeURIComponent(eventData.url)}`;
        ...
        function shareOnFacebook() {
            const shareUrl = `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(eventData.url)}&quote=${encodeURIComponent('Check out this event: ' + eventData.name)}`;
            window.open(shareUrl, '_blank', 'width=600,height=400');
        }

        function shareOnTwitter() {
            const text = `Check out this event: ${eventData.name}`;
            const shareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(eventData.url)}`;
            window.open(shareUrl, '_blank', 'width=600,height=400');
        }

        function shareOnWhatsApp() {
            const text = `Check out this event: ${eventData.name} - ${eventData.url}`;
        ...
        function downloadQRCode() {
            ...
            if (canvas) {
                ...
                link.download = `${eventData.name}-qr-code.png`;
